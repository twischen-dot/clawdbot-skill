...
**重要说明**：此 skill 预设使用 `natuo` 代理名称。如需使用其他代理，请修改 SKILL.md 中的代理名称配置。
...
此 skill 预设使用 `natuo` 代理名称。如需修改为其他代理：

1. 编辑 `~/.claude/skills/clawdbot/SKILL.md`
2. 找到 `--agent natuo` 部分
3. 将 `natuo` 替换为你自己的 Clawdbot 代理名称（如 `main`、`ops`、`assistant` 等）
...
### 示例 4: 状态同步
```
记录状态到 Natuo：任务已进入测试阶段
```

## 工作原理

1. OpenCode 识别用户的通信请求
2. 构建标准命令：`clawdbot agent --agent natuo --message "..."`
3. 执行命令发送消息
4. 返回通信结果
...
默认情况下，skill 使用 `natuo` 作为代理名称。如需修改为其他代理：

1. 查看你的代理列表：`clawdbot agents list`
2. 编辑 `~/.claude/skills/clawdbot/SKILL.md`
3. 将 `--agent natuo` 中的 `natuo` 替换为你的实际代理名
...
```bash
# 修改前（使用 natuo）
clawdbot agent --agent natuo --message "..."
...
- 是否使用正确的代理名称：`--agent natuo`（或你配置的其他代理名）
- 消息是否用双引号正确包裹
...
**Q: 文档中的 "natuo" 是什么？**
A: 这是本 skill 预设使用的代理名称。你可以修改为任何其他你已经配置的 Clawdbot 代理名称。
